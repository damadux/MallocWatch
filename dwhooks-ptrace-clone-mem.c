...
int bounds_tsc,bounds_tsc_high,bounds_tsc_end,bounds_tsc_end_high;
int nb_accesses = 0;

static int malloc_hook_active = 0;
static int malloc_hook_last = 0;

FILE *bench_file_mem_accesses;
FILE *bench_file_disasm;
FILE *bench_file_mem;
...
static int mmap_metadata_size = 2500;
...
mmap_pointer_check_access(const void *addr) {
    for(int i = 0; i < mmap_oid_last; i++) {
        if(addr >= mmap_metadata[i].base_addr && addr < mmap_metadata[i].end) {
            if(pointer_method == 5) {
                in_syscall++;
                // Compute the page to unprotect: unprotecting an entire object when it is very large takes a very long time
                long long unsigned addr_prot = ((((long long unsigned)addr - (long long unsigned)mmap_metadata[i].base_addr) / 4096) * 4096) + (long long unsigned)mmap_metadata[i].base_addr;
                //fprintf(stderr,"check_access %llu,%p,%p",addr_prot,mmap_metadata[i].base_addr, mmap_metadata[i].end);
                //__asm__ __volatile__("rdtsc" : "=a" (bounds_tsc), "=d" (bounds_tsc_high));
                //mprotect(mmap_metadata[i].base_addr, mmap_metadata[i].size, PROT_READ | PROT_WRITE);
                if ((addr_prot + 4096) == (long long unsigned)mmap_metadata[i].end) {
                	mprotect((void *)addr_prot, 4096, PROT_READ | PROT_WRITE);
                }
                else {
                	mprotect((void *)addr_prot, 8192, PROT_READ | PROT_WRITE);
                }
                //__asm__ __volatile__("rdtsc" : "=a" (bounds_tsc_end), "=d" (bounds_tsc_end_high));
                //fprintf(bench_file_disasm, "Unprotect: %u\n",bounds_tsc_end - bounds_tsc);
                in_syscall--;
...
mmap_pointer_resume_access(const void *addr) {
    for(int i = 0; i < mmap_oid_last; i++) {
        if(addr >= mmap_metadata[i].base_addr && addr < mmap_metadata[i].end) {
            if(pointer_method == 5) {
                in_syscall++;
                // Compute the page to protect: protecting an entire object when it is very large takes a very long time
                long long unsigned addr_prot = ((((long long unsigned)addr - (long long unsigned)mmap_metadata[i].base_addr) / 4096) * 4096) + (long long unsigned)mmap_metadata[i].base_addr;
                //__asm__ __volatile__("rdtsc" : "=a" (bounds_tsc), "=d" (bounds_tsc_high));
                //mprotect(mmap_metadata[i].base_addr, mmap_metadata[i].size, PROT_NONE);
                if ((addr_prot + 4096) == (long long unsigned)mmap_metadata[i].end) {
                	mprotect((void *)addr_prot, 4096, PROT_NONE);
                }
                else {
                	mprotect((void *)addr_prot, 8192, PROT_NONE);
                }
                //__asm__ __volatile__("rdtsc" : "=a" (bounds_tsc_end), "=d" (bounds_tsc_end_high));
                //fprintf(bench_file_disasm, "Reprotect: %u\n",bounds_tsc_end - bounds_tsc);
                in_syscall--;
...
mmap_pointer_malloc_hook(size_t size) {
    ...
    mmap_oid_last++;
    unsigned long malloc_tsc, malloc_tsc_high;
    __asm__ __volatile__("rdtsc" : "=a" (malloc_tsc), "=d" (malloc_tsc_high));
    //fprintf(stderr, "%lx\n",malloc_tsc_high);
    malloc_tsc += (malloc_tsc_high << 32);
    //fprintf(stderr, "%lx\n",malloc_tsc);
    fprintf(bench_file_mem,"Malloc,%p,%lu,%lu,%lu\n", addr, size, real_size, malloc_tsc);
    fflush(bench_file_mem);
...
mmap_pointer_free_hook (void *ptr) {
    ...
    for(int i = 0; i < mmap_metadata_size; i++) {
        if(ptr >= mmap_metadata[i].base_addr && ptr < mmap_metadata[i].end) {
            ...
            unsigned long free_tsc, free_tsc_high;
			__asm__ __volatile__("rdtsc" : "=a" (free_tsc), "=d" (free_tsc_high));
			//fprintf(stderr, "free %lx\n",free_tsc_high);
			free_tsc += (free_tsc_high << 32);
			//fprintf(stderr, "%lx\n",free_tsc);
			fprintf(bench_file_mem,"Free,%p,%lu\n", ptr, free_tsc);
...
child_program(void* malloc_addr) {
    ...
    */
    ...
    while(1){
        ...
        nb_accesses++;
        /*
       	if (nb_accesses >= 1000000){
       		fprintf(stderr,"Access limit reached!\n");
       		abort();
       	}
       	*/
        unsigned long start_mem, start_mem_high, end_mem, end_mem_high, start_disasm, start_disasm_high, end_disasm, end_disasm_high;
        int report_addr = 1; // Use this variable to know which address to write as the faulty address (benchmark)
		__asm__ __volatile__("rdtsc" : "=a" (start_mem), "=d" (start_mem_high));
	if (WIFEXITED(status)) {
        ...
        long long unsigned addr, siginfo_addr, base_addr, index_addr, scale, scale_2, displacement, displacement_2;
        long long unsigned op_addr = 0;
        int mem_op_count = 0;
        ...
        if(pointer_method != 3 && address_method == 0) {
    		...
    		//__asm__ __volatile__("rdtsc" : "=a" (start_disasm), "=d" (start_disasm_high));
      	    bool success = cs_disasm_iter(handle, &code , &sizeds, &instr_addr, insn);
      	    //__asm__ __volatile__("rdtsc" : "=a" (end_disasm), "=d" (end_disasm_high));
    	    fprintf(bench_file_disasm, "INFO Signal %d, disasm 0x%llx (%d, %d), 0x%lx: %s %s, (%hu)\n", s, regs.rip, success, cs_errno(handle), insn->address, insn->mnemonic, insn->op_str, insn->size);
            detail = insn->detail;
    	    x86 = &detail->x86;
    	    if(x86->op_count < 1) { fprintf(stderr, "Received SIGSEGV but instruction has no argument\n"); abort(); }

            // There may be 2 memory operands in the same instructions. As such, we should be sure to unprotect all 2 arguments, not only the last one.
    	    ...
    	    for (size_t i=0; i < x86->op_count; i++){
    	        switch(x86->operands[i].type) {
                    ...
                    case X86_OP_MEM:
                    	mem_op_count++;
                    	if (mem_op_count == 2){
		                	base_reg_2 = x86->operands[i].mem.base;
		                    index_reg_2 = x86->operands[i].mem.index;
		                    res_base = get_reg(base_reg_2, &regs, &base_addr);
		                    res_index = get_reg(index_reg_2, &regs, &index_addr);
		                    scale_2 = x86->operands[i].mem.scale;
		                    displacement_2 = x86->operands[i].mem.disp;
		                    op_addr = base_addr + (index_addr * scale) + displacement;
                    		fprintf(stderr,"%llx, %llx, %llx",siginfo_addr, addr, op_addr);
                    		
                    		
                    	}
                    	else if (mem_op_count >= 3){
                    		fprintf(stderr, "ERROR: %u memory operands\n",mem_op_count);
                    	}
                        ...
                        // If this is the second memory operand, siginfo_addr and addr will disagree
                        //if(addr != siginfo_addr && mem_op_count == 1) fprintf(stderr, "Error, siginfo_addr and addr disagree: %llx, %llx\n", siginfo_addr, addr);
        ...
        if(pointer_method == 3) {
            ...
            */
		        ...
		        ptrace(PTRACE_SINGLESTEP,ppid,NULL,NULL);
		        //__asm__ __volatile__("rdtsc" : "=a" (bounds_tsc_end), "=d" (bounds_tsc_end_high));
                //fprintf(bench_file_disasm, "Singlestep: %u\n",bounds_tsc_end - bounds_tsc);
		        waitpid(ptid,&status,0);
		        if(WIFEXITED(status)) break;
		        if(WIFSTOPPED(status)) {
		            int s = WSTOPSIG(status);
        	...
        	if (mem_op_count == 2){
        		...
        		if (mmap_pointer_check_access((void *)op_addr)){
        			first_addr = 1;
        			report_addr += 2;
        		}
        		if (first_addr || second_addr){
        			//fprintf(stderr, "MProtected access %llx and %llx\n", op_addr, addr);
		            ptrace(PTRACE_SINGLESTEP,ppid,NULL,NULL);
		            waitpid(ptid,&status,0);
		            if(WIFEXITED(status)) break;
		            if(WIFSTOPPED(status)) {
		            ...
		            if (first_addr) mmap_pointer_resume_access((void *)addr);
		            if (second_addr) mmap_pointer_resume_access((void *)op_addr);
            ...
            else if(mmap_pointer_check_access((void *)addr)) {
        ...
        ptrace(tracee_continue, ppid, NULL, NULL);
        __asm__ __volatile__("rdtsc" : "=a" (end_mem), "=d" (end_mem_high));
        
        start_mem += (start_mem_high << 32);
        end_mem += (end_mem_high << 32);
		unsigned long diff = end_mem - start_mem;
		switch(report_addr) {
		
			case 1:
				fprintf(bench_file_mem_accesses, "Access,0x%lx,0x%llx,%lu,%lu\n",insn->address,addr,diff,start_mem);
				break;
			case 2:
				fprintf(bench_file_mem_accesses, "Access,0x%lx,0x%llx,%lu,%lu\n",insn->address,op_addr,diff,start_mem);
				break;
			case 3:
			    fprintf(bench_file_mem_accesses, "Access,0x%lx,0x%llx,%lu,%lu\n",insn->address,addr,diff,start_mem);
				//fprintf(bench_file_mem_accesses, "Access,0x%lx,0x%llx,%lu,%lu\n",insn->address,op_addr,diff,start_mem);
        ...
        //start_disasm += (start_disasm_high << 32);
        //end_disasm += (end_disasm_high << 32);
		//diff = end_disasm - start_disasm;
		//fprintf(bench_file_disasm, "Disasm,0x%lx,%lu,%lu\n",insn->address,diff,start_disasm);
        
        
        //fflush(bench_file_mem_accesses);
        //malloc_hook_active = 1;
...
__attribute__((constructor)) dw_init()
{
    ...
    // Bench file
    arg = getenv("DW_BENCH_NUMBER");
    if(arg != NULL) bench_number = atol(arg);
    
    bench_file_mem_accesses = fopen("bench_accesses.txt","w");
    if (!bench_file_mem_accesses) fprintf(stderr,"bench_accesses failed to open");
    bench_file_mem = fopen("bench_mallocs.txt","w");
    if (!bench_file_mem) fprintf(stderr,"bench_malloc failed to open");
    bench_file_disasm = fopen("bench_capstone.txt","w");
    if (!bench_file_disasm) fprintf(stderr,"bench_accesses failed to open");
    fprintf(bench_file_mem, "Run %u, memory accesses: \n\n",bench_number);
